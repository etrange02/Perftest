...
public class TestPlanManagementFacade implements ITestPlanManagement {
	...
	if (null == getTestPlan() || null == test || null == instructionType || instructionType.isEmpty())
	    return null;
    ...
    @Override
    public void replaceInstruction(AbstractMonitoredTest test, int id, 
	    IInstruction instruction) {
	if(test == null || instruction == null || test.getInstructions() == null || 
		id < 0 || test.getInstructions().size() <= id)
	    return;

	if(this.getTestPlan().getTests().contains(test)) {
	    test.getInstructions().set(id, instruction);
	    test.updateDataListeners();
	}
