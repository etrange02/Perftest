...
public class LDAPClientForBlankTest extends AbstractClientForBlankTest {
	...
	try {
	    ...
	    System.out.println("LDAPClientForBlankTest(): "+instructions.size()+" instructions to perform.");
	    ...
	    Thread.sleep(10000); //TODO FIXME: wait more properly for all instructions to finish

	    //TODO it seem that the last instruction (disconnection instruction)
	    //send sometime data to server.
	    IInstruction lastInstruction = instructions.get(instructions.size()-1);

	    if(lastInstruction.getBinaryRequest() == null || 
		    lastInstruction.getBinaryRequest().length == 0) {
